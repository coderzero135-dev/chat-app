...
export default function App() {
  ...
  const requestMicPermission = useCallback(async () => {
    try {
      ...
      setMicGranted(true);
    } catch (_) { setMicGranted(false); }
  }, []);

  const startRecording = useCallback(() => {
    if (!micGranted) { requestMicPermission(); return; }
  ...
  const stopRecording = useCallback(() => {
    ...
    else recordingActive.current = false;
  }, [recordingLocked]);
          ...
          <div className="input-bar">
            ...
            {input.trim() || filePreview ? (
              ...
              <button className={`send-btn mic-btn ${recording ? 'recording' : ''}`}
                ...
                title={micGranted ? "Hold to record, slide up to lock" : "Tap to enable mic, then hold to record"}
